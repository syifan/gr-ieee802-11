#include <gnuradio/ieee802_11/ofdm_symbol_mapper.h>
...
class ofdm_symbol_mapper_impl : public ofdm_symbol_mapper
{


public:
	ofdm_symbol_mapper_impl(const std::vector<gr_complex> &constellation, unsigned int msgq_limit,
					unsigned int occupied_carriers, unsigned int fft_length,bool debug)
	: gr_sync_block ("ofdm_symbol_mapper",
	  ...
	  d_nresid(0)
  {
	if (!(d_occupied_carriers <= d_fft_length))
	  throw std::invalid_argument("ofdm_symbol_mapper: occupied carriers must be <= fft_length");
...
ofdm_symbol_mapper::sptr
ofdm_symbol_mapper::make(const std::vector<gr_complex> &constellation, unsigned msgq_limit,
		   unsigned occupied_carriers, unsigned int fft_length,bool debug) {
	return gnuradio::get_initial_sptr(new ofdm_symbol_mapper_impl(constellation,msgq_limit,occupied_carriers,fft_length,debug));
}
